...
import {API_ORIGIN, ApiResponse} from "@/services/api.ts";
...
export const adminApiService = {
    ...
    regenerateAdminPassword: async (id: string | number) => {
        const token = adminApiService.getToken();
        const response = await axios.post(`${BASE_URL}/management/admins/${id}/regenerate-password`, {}, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        return response.data;
    },

    resendAdminCredentials: async (id: string | number) => {
        const token = adminApiService.getToken();
        const response = await axios.post(`${BASE_URL}/management/admins/${id}/resend-credentials`, {}, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        return response.data;
    },
