import {API_ORIGIN, apiService} from './api';
...
class AdminDocumentService {
    ...
    async downloadDocument(nomFichier: string): Promise<Blob> {
        try {
            ...
            const response = await fetch(`${API_ORIGIN}/documents/${nomFichier}/download`);
            if (!response.ok) {
    ...
    getDocumentPreviewUrl(nomFichier: string): string {
        return `${API_ORIGIN}/documents/${nomFichier}/download`;
    }
