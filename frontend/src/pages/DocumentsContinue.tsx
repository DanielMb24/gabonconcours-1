...
const DocumentsContinue = () => {
    ...
    const uploadMutation = useMutation({
        ...
        onSuccess: (response) => {
            ...
            setTimeout(() => navigate(`/dossier/recap/${encodeURIComponent(decodedNupcan)}`), 800);
        },
        onError: (error) => {
    ...
    const handleContinuer = () => {
        // Si des documents existent déjà, afficher le récapitulatif du dossier.
        if (existingDocuments?.data && existingDocuments.data.length > 0) {
            candidatureStateManager.updateProgression(decodedNupcan, 'documents');
            navigate(`/dossier/recap/${encodeURIComponent(decodedNupcan)}`);
            return;
        }
