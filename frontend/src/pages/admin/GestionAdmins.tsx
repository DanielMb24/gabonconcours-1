...
const GestionAdmins = () => {
        ...
        onSuccess: (response: any) => {
            ...
            toast({
                ...
                description: response.data?.delivery?.emailSent
                    ...
                    : `Mot de passe temporaire à transmettre manuellement : ${response.data?.delivery?.temporaryPassword || 'indisponible'}`,
                variant: response.data?.delivery?.emailSent ? "default" : "destructive",
            });
        },
        onError: (error: any) => {
            toast({
    ...
    const regeneratePasswordMutation = useMutation({
        mutationFn: adminApiService.regenerateAdminPassword,
        onSuccess: (response: any) => {
            setCredentialResult({email:response.data.admin.email,password:response.data.temporaryPassword,title:'Nouveau mot de passe généré'});
            toast({title:'Mot de passe régénéré',description:'Copiez-le maintenant : il ne sera plus affiché ensuite.'});
        },
        onError: (error:any) => toast({title:'Erreur',description:error.message||'Régénération impossible',variant:'destructive'}),
    });
    const resendCredentialsMutation = useMutation({
        mutationFn: adminApiService.resendAdminCredentials,
        onSuccess: (response:any) => {
            if(response.data.delivery.emailSent) toast({title:'Identifiants envoyés',description:`Un nouveau mot de passe a été envoyé à ${response.data.admin.email}.`});
            else setCredentialResult({email:response.data.admin.email,password:response.data.delivery.temporaryPassword,title:'Email non envoyé — transmission manuelle requise'});
        },
                                        ...
                                        <div className="flex items-center space-x-2">
                                            {admin.role !== 'super_admin' && (
                                                <>
                                                    ...
                                                    <Button variant="ghost" size="sm" title="Renvoyer les identifiants" onClick={()=>resend(admin)} disabled={resendCredentialsMutation.isPending}>
                                                        <Mail className="h-4 w-4"/>
                                                    </Button>
                                                    <Button variant="ghost" size="sm" title="Régénérer le mot de passe" onClick={()=>regenerate(admin)} disabled={regeneratePasswordMutation.isPending}>
                    ...
                    {selectedAdmin && (
                        <form onSubmit={handleEditAdmin} className="space-y-4">
                            ...
                            <div className="border-t pt-4 flex flex-wrap gap-2">
                                <Button type="button" variant="outline" onClick={()=>resend(selectedAdmin)} disabled={resendCredentialsMutation.isPending}><Mail className="h-4 w-4 mr-2"/>Renvoyer les identifiants</Button>
                                <Button type="button" variant="outline" onClick={()=>regenerate(selectedAdmin)} disabled={regeneratePasswordMutation.isPending}><KeyRound className="h-4 w-4 mr-2"/>Régénérer le mot de passe</Button>
                            </div>
                        </form>
                    )}
                </DialogContent>
            </Dialog>
            <Dialog open={Boolean(credentialResult)} onOpenChange={(open)=>!open&&setCredentialResult(null)}>
                <DialogContent>
                    <DialogHeader><DialogTitle>{credentialResult?.title}</DialogTitle></DialogHeader>
                    <p className="text-sm text-muted-foreground">Ce mot de passe temporaire n'est affiché qu'une seule fois.</p>
                    <div className="rounded-lg border bg-muted/40 p-4 space-y-2">
                        <div><span className="font-medium">Email :</span> {credentialResult?.email}</div>
                        <div className="flex items-center justify-between gap-3"><code className="text-base font-semibold break-all">{credentialResult?.password}</code><Button type="button" size="sm" variant="outline" onClick={()=>navigator.clipboard.writeText(credentialResult?.password||'')}><Copy className="h-4 w-4 mr-1"/>Copier</Button></div>
                    </div>
                    <Button onClick={()=>setCredentialResult(null)}>J'ai enregistré le mot de passe</Button>
                </DialogContent>
            </Dialog>
        </div>
    );
