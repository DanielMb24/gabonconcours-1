                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <Card>
                                <CardContent className="pt-4 space-y-4">
                                    ...
                                    <div className="pt-4 border-t">
                                        <Label className="text-sm font-bold text-gray-600">Assistance IA</Label>
                                        <p className="mt-2 text-sm text-muted-foreground">
                                            {document.ai_status === 'pending' || document.ai_status === 'running'
                                                ? 'Analyse automatique en cours.'
                                                : document.ai_status === 'failed'
                                                    ? 'Analyse automatique indisponible.'
                                                    : document.ai_recommendation === 'approve'
                                                        ? `Recommandation : valider${document.ai_confidence != null ? ` (${Math.round(document.ai_confidence * 100)} % de confiance)` : ''}.`
                                                        : document.ai_recommendation === 'reject'
                                                            ? `Recommandation : rejeter${document.ai_confidence != null ? ` (${Math.round(document.ai_confidence * 100)} % de confiance)` : ''}.`
                                                            : document.ai_recommendation === 'review'
                                                                ? 'Recommandation : vérification humaine requise.'
                                                                : 'Aucune recommandation disponible.'}
                                        </p>
                                        {document.ai_reason && <p className="mt-1 text-xs text-muted-foreground">{document.ai_reason}</p>}
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
