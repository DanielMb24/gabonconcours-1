...
export interface ConcoursFormData {
    ...
    documents_requis: Array<{
        ...
        obligatoire: boolean;
        description: string;
        ia_indications_validation: string;
        ia_indications_rejet: string;
    }>;
...
const DOCUMENTS_DEFAULT = [
    { nom: 'Acte de naissance', obligatoire: true, description: 'Acte de naissance original ou copie certifiée', ia_indications_validation: '', ia_indications_rejet: '' },
    { nom: 'Certificat de nationalité', obligatoire: true, description: 'Certificat de nationalité gabonaise', ia_indications_validation: '', ia_indications_rejet: '' },
    { nom: 'Diplôme du Baccalauréat', obligatoire: true, description: 'Diplôme du Baccalauréat ou équivalent', ia_indications_validation: '', ia_indications_rejet: '' },
    { nom: 'Relevé de notes du Baccalauréat', obligatoire: true, description: 'Relevé de notes complet', ia_indications_validation: '', ia_indications_rejet: '' },
    { nom: 'Photo d\'identité', obligatoire: true, description: 'Photo d\'identité récente (format 4x4)', ia_indications_validation: '', ia_indications_rejet: '' },
    { nom: 'Certificat médical', obligatoire: true, description: 'Certificat médical de moins de 3 mois', ia_indications_validation: '', ia_indications_rejet: '' },
    { nom: 'Casier judiciaire', obligatoire: true, description: 'Bulletin n°3 du casier judiciaire', ia_indications_validation: '', ia_indications_rejet: '' },
];
...
export const CreateConcoursMultiStep: React.FC<CreateConcoursMultiStepProps> = ({ onClose, onSuccess, mode = 'create', concoursId, initialData }) => {
    ...
    const [formData, setFormData] = useState<ConcoursFormData>(() => ({
        ...
        ...(initialData ? {
            ...
            series_bac_acceptees: parseList(initialData.series_bac_acceptees), documents_requis: parseList(initialData.documents_requis).map((item: any) => ({ nom: item.nom || item.name || '', obligatoire: item.obligatoire !== false && item.required !== false, description: item.description || '', ia_indications_validation: item.ia_indications_validation || item.aiValidationInstructions || '', ia_indications_rejet: item.ia_indications_rejet || item.aiRejectionRules || '' })),
    ...
    const addDocument = () => {
        setFormData({
            ...
            documents_requis: [
                ...
                { nom: '', obligatoire: true, description: '', ia_indications_validation: '', ia_indications_rejet: '' }
            ]
        });
    };
                        ...
                        <CardContent className="pt-4">
                            <div className="space-y-3">
                                ...
                                <Textarea
                                    ...
                                    onChange={(e) => updateDocument(index, 'ia_indications_validation', e.target.value)}
                                />
                                <Textarea
                                    placeholder="Consignes IA pour rejeter (ex. document expiré, page manquante, mauvaise pièce)"
