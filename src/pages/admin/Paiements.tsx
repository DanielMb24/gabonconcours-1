...
const Paiements = () => {
    ...
    const handleValidatePayment = async (paiementId: string | number) => {
        const response = await apiService.makeRequest(`/paiements/${paiementId}/status`, 'PATCH', {statut:'valide'});
        toast({title: response.success ? "Paiement validé" : "Erreur",description:response.message,variant:response.success?'default':'destructive'});
    };
                                ...
                                <TableBody>
                                    {filteredPaiements.length > 0 ? filteredPaiements.map((paiement) => (
                                        <TableRow key={paiement.id} className="hover:bg-muted/50">
                                            ...
                                            <TableCell>
                                                <div className="flex items-center space-x-2">
                                                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0" title="Afficher les informations" onClick={()=>toast({title:`Paiement ${paiement.reference}`,description:`${paiement.candidat_nom} · ${paiement.concours} · ${(paiement.montant||0).toLocaleString('fr-FR')} FCFA`})}>
