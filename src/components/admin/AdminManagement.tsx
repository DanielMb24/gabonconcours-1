        ...
        try {
            if (editingAdmin) {
                ...
                if (response.data.success) {
                    ...
                    toast({
                        title: delivery?.emailSent ? 'Administrateur créé' : 'Administrateur créé, email non envoyé',
                        description: delivery?.emailSent
                            ? 'Les identifiants ont été envoyés par email.'
                            : `Mot de passe temporaire à transmettre manuellement : ${delivery?.temporaryPassword || 'indisponible'}`,
                        variant: delivery?.emailSent ? 'default' : 'destructive',
                    });
                }
            }
