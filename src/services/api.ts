...
const DEFAULT_API_URL = import.meta.env.PROD
    ? 'https://api-econcour.vercel.app/api/v1'
    : 'http://localhost:3001/api/v1';
export const API_ORIGIN = (import.meta.env.VITE_API_URL || DEFAULT_API_URL).replace(/\/$/, '');
export const BACKEND_ORIGIN = API_ORIGIN.replace(/\/api\/v1$/, '');
...
export class ApiService {
    delete<T = unknown>(url: string, config?: { data?: unknown }): Promise<ApiResponse<T>> {
        return this.makeRequest<T>(url, 'DELETE', config?.data);
    }
