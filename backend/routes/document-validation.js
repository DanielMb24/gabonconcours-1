...
router.put('/:id', async (req, res) => {
    try {
        ...
        let [rows] = await connection.execute?.(
            `SELECT d.*, dos.nupcan AS candidat_nupcan, dos.id AS dossier_id, dos.document_id AS dossier_document_id
             FROM documents d
        ...
        const nupcan = document.candidat_nupcan || document.nupcan;
        if (nupcan) {
            await connection.execute(
                `INSERT INTO notifications (candidat_nupcan, type, titre, message, statut, created_at)
                 VALUES (?, ?, ?, ?, 'non_lu', NOW())`,
                [nupcan, statut === 'valide' ? 'document_validation' : 'document_rejection',
                    statut === 'valide' ? 'Document validé' : 'Document rejeté',
                    statut === 'valide'
                        ? 'Votre document a été validé.'
                        : `Votre document a été rejeté. ${commentaire || ''}`]
            );
        }
